...
from models import setup_db, Greeting, db_drop_and_create_all
...
def create_app(test_config=None):
    ...
    # NOTE:  uncomment when running for the first time
    # then comment it and run again
    ...
    def greeting_all(payload):
        ...
        total = len(greetings)
        print("greets ", greetings)
        greetings = [greeting.format() for greeting in greetings]
        return jsonify({'greetings': greetings[((page-1)*Page_count):((page)*Page_count)], 'total': total})
